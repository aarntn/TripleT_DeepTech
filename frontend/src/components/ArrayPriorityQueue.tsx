    ...
    <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
      <div className="flex flex-col gap-3 border-b border-slate-200 bg-slate-50 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <h2 className="mt-1 text-xl font-bold text-slate-950">Work-order decisions by array</h2>
          <p className="mt-1 text-sm text-slate-500">
            Sorted by dust likelihood and RM loss so O&amp;M teams can dispatch with justification.
          </p>
        </div>
        <button
          type="button"
          ...
          className="rounded-md bg-emerald-600 px-4 py-2 text-sm font-bold text-white shadow-sm transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:bg-slate-300"
        >
          Schedule all dust cleaning
        </button>
      </div>
        ...
        <table className="w-full min-w-[820px] text-left text-sm">
          <thead className="bg-white text-xs uppercase tracking-wide text-slate-500">
            <tr>
              ...
              <th className="px-4 py-3">RM loss today</th>
              ...
              return (
                ...
                >
                  <td className="px-4 py-3">
                    <p className="font-bold text-slate-950">{panel.id}</p>
                    <p className="text-xs text-slate-500">{panel.name}</p>
                  </td>
                  <td className={`px-4 py-3 font-bold ${statusStyles[status]}`}>{status}</td>
                  <td className="px-4 py-3">
                    <span className={`rounded-full border px-2.5 py-1 text-xs font-bold ${causeStyles[panel.classifier.type]}`}>
                      {panel.classifier.type}
                    </span>
                  </td>
                  <td className="px-4 py-3 font-semibold text-slate-700">{panel.classifier.confidence}%</td>
                  <td className="px-4 py-3 font-bold text-rose-600">{formatRM(panel.lossToday)}</td>
                  <td className="px-4 py-3">
                    {cleanable ? (
                      <button
                        type="button"
                      ...
                      >
                        {cleaningIds.has(panel.id) ? "Scheduling..." : "Schedule cleaning"}
                      </button>
